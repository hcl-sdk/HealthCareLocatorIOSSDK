...
class SegmentControlView: UIView {
    ...
    private var itemViews: [SegmentControlItem] = []
    weak var delegate: SegmentControlViewProtocol? {
        didSet {
    ...
    init(items: [SegmentControlModel], selectedIndex: Int) {
    ...
    private func initialize() {
        if let nibView = Bundle.internalBundle().loadNibNamed("SegmentControlView", owner: self, options: nil)?.first as? UIStackView {
    ...
    private func layoutWith(items: [SegmentControlModel], selectedIndex: Int) {
        // Clean sub views
        itemViews = []
        ...
        for (index, item) in items.enumerated() {
            let itemView = SegmentControlItem(item: item,
                                                index: index,
